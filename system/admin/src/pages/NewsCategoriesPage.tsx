...
import { ChevronDown, ChevronRight, FolderTree, Loader2, Plus, Trash2 } from 'lucide-react'
...
type CategoryForm = Pick<NewsCategory, 'name' | 'parent_id' | 'sort_order'>

interface CategoryNode extends NewsCategory {
...
export default function NewsCategoriesPage() {
  ...
  const categories = useMemo(() => (data?.data || []).filter((category) => category.id !== 0), [data])
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, value }: { id: number; value: CategoryForm }) => {
      const response = await newsCategoriesApi.update(id, value)
  ...
  const createMutation = useMutation({
    mutationFn: async (value: CategoryForm) => {
      const response = await newsCategoriesApi.create(value)
      if (!response.data) throw new Error('服务器未返回分类数据')
      return response.data
    },
    onSuccess: (created) => {
  ...
  const saveCurrentForm = (nextForm = formData) => {
    if (updateMutation.isPending || createMutation.isPending || !formDirtyRef.current) return
  ...
  return (
    <div className="flex h-full min-h-[calc(100vh-8rem)] flex-col gap-4 lg:flex-row">
      ...
      <Card className="min-h-0 min-w-0 flex-1">
        <CardHeader className="border-b">
          <CardTitle>{draftParentId !== null ? '新增新闻分类' : selectedCategory?.name || '分类编辑'}</CardTitle>
          <CardDescription>
            {draftParentId !== null
              ? '填写分类名称并移开焦点后自动创建'
              : selectedCategory
          ...
          {selectedCategory || draftParentId !== null ? (
            <form className="flex max-w-3xl flex-col gap-5" onSubmit={(event) => event.preventDefault()}>
              <div className="space-y-2">
                <Label htmlFor="news-category-name">分类名称 *</Label>
                <Input
              ...
              <div className="space-y-2">
                <Label htmlFor="news-category-parent">父分类</Label>
                <Select value={String(formData.parent_id)} onValueChange={(value) => updateSelectField(Number(value))}>
                  <SelectTrigger id="news-category-parent" className="w-full">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
              ...
              <div className="space-y-2">
                <Label htmlFor="news-category-sort">排序</Label>
                <Input
                  id="news-category-sort"
                  type="number"
      ...
      <AlertDialog open={Boolean(deleteTarget)} onOpenChange={(open) => !open && setDeleteTarget(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              确定要删除「{deleteTarget?.name}」吗？此操作无法撤销；如果分类下有子分类或新闻，删除可能会失败。
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
...
function CategoryTree({
  ...
  onAddChild: (category: NewsCategory) => void
...
function buildCategoryTree(categories: NewsCategory[]) {
  const nodes = new Map<number, CategoryNode>()
  const roots: CategoryNode[] = []
  categories.forEach((category) => nodes.set(category.id, { ...category, children: [], depth: 0 }))
  nodes.forEach((node) => {
...
function assignDepth(nodes: CategoryNode[], depth: number) {
  nodes.forEach((node) => {
    node.depth = depth
...
function collectDescendantIds(id: number, categories: NewsCategory[]) {
...
function toCategoryForm(category: NewsCategory): CategoryForm {
  return {
    name: category.name || '',
